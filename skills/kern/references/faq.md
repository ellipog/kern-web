<!-- generated from content/docs/faq.md - edit the source in kern-web, then run: npm run skill:build -->
...
**where does my data live?** the app data dir (settings/plugins) and your instance folders (logs/backups/worlds). see [config.json](./config-json.md). deleting kern never deletes your servers.
...
**can i control it from my phone?** yes — the [web remote](./web-remote.md): https + qr pairing on your lan.

**can i script it?** yes — [`kern-cli`](./cli.md) for shells, and the [automation api](./automation-api.md) for anything else, both on loopback with a bearer token.
...
**can a plugin read my other servers or files outside its instance?** only if you granted `servers:read` / `files:read` and it uses them; file commands resolve paths under server directories. see [plugin security](./plugin-security.md).
