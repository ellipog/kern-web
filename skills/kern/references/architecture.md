<!-- generated from content/docs/architecture.md - edit the source in kern-web, then run: npm run skill:build -->
...
a `.kern` package is a zip: `manifest.json` + an optional esm ui bundle. the host mounts plugin ui in a **shadow root** (styles isolated), and every host call goes through `HostAPI.invoke`, an allowlist keyed by manifest permissions. see the plugin development section and [plugin security](./plugin-security.md).
